(() => {  
  ...
  function showState(state) {  
    // states: "loading", "no-results", "error", "content"  
    hideAllStates();  
    // hide main content when showing a state other than content  
    if (state === "content") {  
      mainContainer && (mainContainer.style.display = "block");  
    } else {  
      mainContainer && (mainContainer.style.display = "none");  
    }  
  ...
  function showLoading() {
    document.getElementById("loading-state").classList.remove("state-hidden");

    // Disable current weather
    document.querySelector(".current-weather-content").style.opacity = "0.3";

    // Hourly skeleton
    document.getElementById("hourly-skeleton").classList.remove("state-hidden");
    document.querySelector(".hourly-list").classList.add("state-hidden");

    // Set info cards to blank state
    document.querySelectorAll(".info-card h4").forEach(el => el.textContent = "—");
...
function hideLoading() {
    document.getElementById("loading-state").classList.add("state-hidden");

    // Re-enable current weather
    document.querySelector(".current-weather-content").style.opacity = "1";

    // Restore hourly
    document.getElementById("hourly-skeleton").classList.add("state-hidden");
    document.querySelector(".hourly-list").classList.remove("state-hidden");

    // Remove skeletons from daily items
    document.querySelectorAll(".daily-item").forEach(el => {
